...
export const createNewItemService =async (food_details:FoodItemInput , imageUrl:string)=>{
    ...
    const query = `INSERT INTO food_items
                ...
                `
    const values = [name, description , price , category , is_available , is_vegetarian , imageUrl]
...
export const toogleStatusOfFoodService  =async(foodid:string , status:boolean) =>{
try {
    const query = `
        UPDATE food_items 
        SET is_available = $1 
