...
export const checkIfItemExistsInCart = async (userId: any, foodId: any) => {
...
export const editQuantityOfItemInCart = async (
  userId: any,
  foodId: any,
  quantity: any,
) => {
...
export const removeItemFromCart = async (userId: any, foodItemId: any) => {
  const query = `
        DELETE FROM cart 
...
export const getUserCartDetails = async (userId: any) => {
  ...
  try {
    ...
    const total_amount = cart_items.reduce(
      (acc, item) => Number(item.total_price) + acc,
      0,
    );
